...
class WeiTouTiao {
    ...
    var relateNewsCellHeight: CGFloat? {
        let size = CGSize(width: KScreenWidth - 30 * 2, height: CGFloat(MAXFLOAT))
    ...
    var homeCellHeight: CGFloat? {
        ...
        let imageW = (KScreenWidth - 2 * KMargin - 2 * 6) / 3
        if let hasImage = has_image  {
            if hasImage { // 说明有图片
                if image_list.count > 0 {
                    ...
                    if large_image_list.count > 0 {
                        let largeImageW = KScreenWidth - 2 * KMargin
            ...
            if thumb_image_list.count != 0 {
                // 1 or 2
                let imageWidth1or2 = (KScreenWidth - KMargin * 2 - 6) * 0.5
                // >= 3
                let imageH = (KScreenWidth - KMargin * 2 - 12) / 3
    ...
    var contentH: CGFloat? {
        ...
        let height = content.getTextHeight(width: KScreenWidth - KMargin * 2)
        return height
    }
    ...
    var title: NSString?
    var titleH: CGFloat? {
        return title?.getTextHeight(width: KScreenWidth - KMargin * 2)
    }
    var newDetailTitleHeight: CGFloat? {
        let size = CGSize(width: KScreenWidth - 2 * KMargin, height: CGFloat(MAXFLOAT))
    ...
    var cellH: CGFloat? {
        ...
        if let videoDetailInfo = video_detail_info {
            let width = KScreenWidth - KMargin * 2
